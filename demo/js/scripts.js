$(function () {
    ...
    function setup() {
        ...
        var tokens = [{
            ...
            { key: "genre", type: "string", options: { values: [{text : "Action", type :  'string', col : 'Demo'}] } },
    ...
    function setFirstSearchShadow() {
        if (options.length > 0 && options[0]['full'].startsWith(oldSearch)) {
            $('#searchShadow').val(options[0]['full']);
        } else {
    ...
    function search(q) {
        ...
        service.search(q).then(function (result) {
            $(".loader").hide();
            console.info(result.autocomplete);
            ...
            $.each(options, function (index, value) {
                $('#suggestions').append($("<li>").text(value.full).click(function () {
                    $('#search').val(value.full + " ");
                    search(value.full);
                    $("#search").focus();
                }));
            });
